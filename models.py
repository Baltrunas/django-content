...
class Page(TreeNode):
    ...
    # Content
    img = models.FileField(_("Image"), upload_to=upload_to, blank=True)
    ...
    slug = models.SlugField(_("Slug"), max_length=128, unique=True, blank=True, default="")
    ...
    #         self.header = self.name
        ...
        slugs = [Page.objects.get(id=item).slug for item in self.tree_path]
        if settings.APPEND_SLASH:
            slugs.append("")
        self.url = os.path.join("/", *slugs)
        super(Page, self).save(*args, **kwargs)
...
#
#     # order
#     sorting = models.PositiveIntecd gerField(_("Sorting"), default=500)
#
#     # Area (choices?)
